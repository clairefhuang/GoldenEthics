...
const AddEditModal: React.FC<AddEditModalProps> = ({ isOpen, onClose, onSave, publication }) => {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      department_name: publication?.department_name || 'Computer Science and Engineering',
      college_or_school: publication?.college_or_school || 'College of Engineering',
      title: publication?.title || '',
      year: publication?.year || new Date().getFullYear(),
    });
    setErrors({});
  }, [isOpen]); // No dependency on publication!
  ...
  const validate = () => {
    ...
    if (!formData.year || isNaN(Number(formData.year))) newErrors.year = 'Please enter a valid year.';
    setErrors(newErrors);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={publication ? 'Edit Publication' : 'Add New Publication'}>
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex items-center justify-end p-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 rounded-b-lg">
          <button type="button" onClick={onClose} className="text-gray-700 bg-white hover:bg-gray-100 border border-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-3 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:bg-gray-600">Cancel</button>
          <button type="submit" className="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:focus:ring-blue-800">{publication ? 'Save Changes' : 'Add Record'}</button>
        </div>
      </form>
    </Modal>
  );
